  ...
  return (
    <div className={`popup popup__${props.name} ${props.isOpen ? 'popup_opened' : ''}`} role="dialog" >
      ...
      <button className="button button_type_close opacity" aria-label="Закрыть" type="button" onClick={props.onClose}></button>
        ...
        <form className="popup__form popup__form_add-new-card" method="POST" name={props.name}>
          {props.children}
          <button type="submit"  className="button popup__button button_type_submit opacity">{props.buttonName}</button>
        </form>
      </div>
    </div>
  );
